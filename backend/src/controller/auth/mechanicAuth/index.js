  ...
  mechRegister: async function (req, res) {
    try {
      ...
      const schema = Joi.object({
        ...
        NationalId: Joi.string().min(7).max(8).required(),
      ...
      check = await MechanicModel.findOne({ where: { NationalId: payload.NationalId } });
      if (check) {
        return res
          .status(400)
          .json({ message: "User with this NationalId already exists" });
      }
