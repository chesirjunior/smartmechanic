...
const Header = () => {
  ...
  const handleLogout = async () => {
    try {
      const res = await axios.post(`http://localhost:3308/logout`);
      console.log(res);
      localStorage.removeItem("token", gettoken);
      localStorage.removeItem("userId", id);
      localStorage.removeItem("userEmail", Email);
      alert("Logged out Successfully.");
      setIsLoggedIn(false);
    } catch (err) {
      console.log("There is an issue in logout", err);
    }
  ...
  return (
    <>
      <Navbar expand="sm" className="bg-blue-500 " fixed= "top">
        <Container fluid>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="mx-auto">
              ...
              <NavLink to="/staff" className="nav-link text-white">
                <b className="hover:border-b-4 border-yellow-300">Mechanics</b>
              </NavLink>
